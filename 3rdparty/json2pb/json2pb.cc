...
static json_t *_pb2json(const Message &msg) {
  ...
  for (size_t i = 0; i != fields.size(); i++) {
    ...
    if (field->is_repeated()) {
      ...
      for (size_t j = 0; j < count; j++) json_array_append_new(array.ptr, _field2json(msg, field, j));
      jf = array.release();
...
static void _json2pb(Message &msg, json_t *root) {
  ...
  for (void *i = json_object_iter(root); i; i = json_object_iter_next(root, i)) {
    ...
    if (field->is_repeated()) {
      ...
      for (size_t j = 0; j < json_array_size(jf); j++) _json2field(msg, field, json_array_get(jf, j));
    } else
